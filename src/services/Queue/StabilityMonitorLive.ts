 ...
 */
export const StabilityMonitorLive: Layer.Layer<StabilityMonitor, PersistenceError> = Layer.effect(
  StabilityMonitor,
  Effect.gen(function* () {
    ...
    const calculateAverageProcessingTime = (): Effect.Effect<number, never> =>
      Effect.gen(function* () {
        try {
          const sessionId = yield* persistence.getCurrentSession()
          const completedTasks = yield* persistence.loadCompletedTasks(sessionId, 10) // Last 10 tasks
    ...
    const performRecoveryActions = (healthStatus: HealthMetrics): Effect.Effect<void, never> =>
      Effect.gen(function* () {
        ...
        Effect.catchAll((error) => 
          Effect.log(`Recovery action failed: ${error instanceof Error ? error.message : String(error)}`)
        )
      )
    ...
    const heartbeatFiber = yield* Effect.gen(function* () {
      ...
      Effect.catchAll((error) => 
        Effect.gen(function* () {
          yield* Effect.log(`Heartbeat fiber failed: ${error instanceof Error ? error.message : String(error)}`)
