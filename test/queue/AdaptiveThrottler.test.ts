    ...
    it("should limit concurrent filesystem operations", async () => {
      const results = await Effect.gen(function*() {
        ...
        const operations = Array.from({ length: 10 }, (_, i) =>
          throttler.throttle("filesystem", Effect.succeed(`fs-op-${i}`))
        )
    ...
    it("should limit concurrent network operations", async () => {
      const results = await Effect.gen(function*() {
        ...
        const operations = Array.from({ length: 15 }, (_, i) =>
          throttler.throttle("network", Effect.succeed(`net-op-${i}`))
        )
    ...
    it("should propagate operation errors", async () => {
      await expect(
        Effect.gen(function*() {
          ...
          yield* throttler.throttle("computation", failingOperation)
        }).pipe(
          Effect.provide(testLayer),
          Effect.provide(TestContext.TestContext),
          Effect.runPromise
